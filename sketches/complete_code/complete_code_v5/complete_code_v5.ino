...
void loop() {
  ...
  if (abs(pwm) < 50) pwm = 0; // deadband, small outputs close to zero are nulled to prevent the robot from buzzing or creeping due to motor friction, imbalance, or sensor noise
...
ISR(TIMER1_COMPA_vect) {
  ...
  errorSum += error * sampleTime; // numerically integrate the error over time

  // Crude Anti-Windup for Ki
